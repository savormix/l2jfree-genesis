 ...
 * Super User - A super user can always login to a game server, provided
 * the game server itself is not offline. In other words, this account can
 * login during maintenance as well as into a full game server.
 ...
 * Ban Reason - ID that defines why an account has been suspended and can
 ...
 * authData - A randomly generated byte array used to distinguish authorized
 * game servers.
 * allowBans - Whether to allow account bans requested by this game server.
 */
-- Table must not be preloaded.
